...
export class ChatService {
  ...
  private async toolGetReorderRecommendation(tenantId: string, query: string) {
    ...
    if (bestSupplier) {
      ...
      // Be explicit so the answer never reads like a hallucination: distinguish
      // "no supplier data on record" from "a supplier exists but is out of stock".
      facts.push({ label: 'الموردون', value: 'لا توجد عروض موردين مسجّلة لهذا المنتج بعد — أضف مورداً أو اطلبه من السوق' });
    }
    if (savingsPct > 0) {
    ...
    return {
      toolResult: {
        ...
        activeSuppliers,
        supplierDataAvailable: activeSuppliers > 0,
        bestSupplier,
        savingsPct,
        note: activeSuppliers === 0
          ? 'no_supplier_data_on_record'
          : undefined,
      },
      cards,
    };
